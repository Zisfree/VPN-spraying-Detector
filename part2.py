...
# 1. Normal Baseline Traffic (Random activity)
for i in range(num_logs - 50):
    ...
    records.append({
        "timestamp": t,
        "user_id": f"user_{random.randint(1000, 1080)}",
...
# 2. Planted Attack: Brute Force (T1110.001) - 1 IP attacks 1 user 15 times
...
# 3. Planted Attack: Password Spray (T1110.003) - 1 IP attacks 15 different accounts
spray_time = base_time + timedelta(minutes=90)
for i in range(15):
    records.append({
        "timestamp": spray_time + timedelta(seconds=i * 5),
        "user_id": f"user_{1010 + i}",
        "src_ip": "185.220.101.44",
        "city": "London, UK",
        "lat": 51.5074,
        "lon": -0.1278,
        "action": "SUCCESS" if i == 14 else "FAILURE",  # Compromised account on 15th try
        "auth_method": "Password",
        "vpn_server": "VPN-02",
        "is_attack": 1
    })
...
print("Updated auth_logs.csv with full schema and planted attacks generated successfully!")
